...
$(function() {
	...
	// 主界面
	var mainDiv = $("#tabContent #country");
	...
	function addData(data) {
		...
		$.get("json/base/country/add.json", data, function(ret) {
			if (ret.success) {
	...
	function modifyData(data) {
		$.get("json/base/country/modify.json", data, function(ret) {
			if (ret.success) {
	...
	function setEnable(id, enable) {
		...
		$.get("json/base/country/enable.json", data, function(ret) {
			if (ret.success) {
	...
	btn_delete.click(function() {
		...
		showConfirmDialog(mainDiv, "请确认", "确认删除选中的行？<br><span class='red'><span class='glyphicon glyphicon-warning-sign'> </span> 该操作不可逆，将进行物理删除！</span>",
				null, function() {
					...
					$.get("json/base/country/delete.json", data, function(ret) {
						if (ret.success) {
	...
	function querySize(callBack) {
		// 查询总记录数
		$.get("json/base/country/selectSize.json", formValue, function(ret) {
			if (ret.success) {
				debugger;
				// 生成页码
	...
	function queryData(page) {
		$.get("json/base/country/select"+(page==null?0:(page%3))+".json", formValue + "&pageSize=" + pageSizeSelect.val() + "&pageNum=" + page, function(ret) {
			if (ret.success) {// 查询成功
	...
	function loadData(data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			tbody += "<td>" + data[i].countryAreaId + "</td>";
			tbody += "<td>" + data[i].code + "</td>";
			tbody += "<td>" + data[i].name + "</td>";
			tbody += "<td>" + data[i].ename + "</td>";
			tbody += "<td>" + data[i].phoneAreaCode + "</td>";
			tbody += "<td>" + data[i].language + "</td>";
			tbody += "<td>" + data[i].currencyCode + "</td>";
			tbody += "<td>" + data[i].currencyName + "</td>";
			tbody += "<td>" + data[i].timeDifference + "</td>";
