$(function() {
	...
	function addData(data) {
		...
		$.get("json/base/china/add.json", data, function(ret) {
			if (ret.success) {
	...
	function modifyData(data) {
		$.get("json/base/china/modify.json", data, function(ret) {
			if (ret.success) {
	...
	function setEnable(id, enable) {
		...
		$.get("json/base/china/enable.json", data, function(ret) {
			if (ret.success) {
	...
	btn_delete.click(function() {
		...
		showConfirmDialog(mainDiv, "请确认", "确认删除选中的行？<br><span class='red'><span class='glyphicon glyphicon-warning-sign'> </span> 该操作不可逆，将进行物理删除！</span>",
				null, function() {
					...
					$.get("json/base/china/delete.json", data, function(ret) {
						if (ret.success) {
	...
	function querySize(callBack) {
		// 查询总记录数
		$.get("json/base/china/selectSize.json", formValue, function(ret) {
			if (ret.success) {
				// 生成页码
	...
	function queryData(page) {
		$.get("json/base/china/select"+(page==null?0:(page%4))+".json", formValue + "&pageSize=" + pageSizeSelect.val() + "&pageNum=" + page, function(ret) {
			if (ret.success) {// 查询成功
	...
	function loadData(data) {
		...
		for (var i = 0; i < data.length; i++) {
			...
			tbody += "<td>" + data[i].areaCnId + "</td>";
			tbody += "<td>" + data[i].code + "</td>";
			tbody += "<td>" + data[i].parentCode + "</td>";
			tbody += "<td>" + data[i].name + "</td>";
			tbody += "<td>" + data[i].level + "</td>";
			tbody += "<td>" + (data[i].ifFinal == 1 ? "<span class='label label-success'>1 是</span>" : "<span class='label label-danger'>0 否</span>")
			+ "</td>";
